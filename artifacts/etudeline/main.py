...
from models import (
    ...
    SiteSettings as SiteSettingsDB
)
...
def _migrate_parametres_universite(db: Session):
    """Ajouter les colonnes messages_actifs, cours_en_ligne_actifs et paiement_masque si elles n'existent pas."""
    new_cols = {
        "messages_actifs": "BOOLEAN NOT NULL DEFAULT TRUE",
        "cours_en_ligne_actifs": "BOOLEAN NOT NULL DEFAULT TRUE",
        "paiement_masque": "BOOLEAN NOT NULL DEFAULT FALSE",
    }
    try:
...
async def login(
    ...
    """Process login with PostgreSQL authentication — role auto-detected from DB"""
    ...
    user_type, _ = auth_result

    # Create session and redirect based on detected role
...
async def serve_uploaded_file(file_path: str, request: Request):
    """Serve uploaded files with Range request support for streaming (PDF, video, audio)"""
    import mimetypes
    ...
    # Handle HTTP Range requests for streaming (PDF.js, video players)
    if range_header:
        try:
            ...
            return Response(
                content=iter_file_range(file_location, range_start, content_length),
                status_code=206,
    ...
    # Full file response with Accept-Ranges declared so browsers/pdf.js can use it
    return FileResponse(
        path=file_location,
        media_type=mime_type,
        headers={
...
async def chapitre_detail_etudiant(chapitre_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    # Vérifier si le paiement est masqué pour l'université de l'étudiant
    paiement_masque = False
    if student.get("universite_id"):
        _p = db.query(ParametreUniversiteDB).filter_by(universite_id=student["universite_id"]).first()
        if _p:
            paiement_masque = getattr(_p, 'paiement_masque', False)
    ...
    return templates.TemplateResponse("chapitre_detail.html", {
        ...
        "paiement_masque": paiement_masque,
    })
...
@app.get("/api/parametres/paiement")
async def get_paiement_status(request: Request, universite_id: str = None, db: Session = Depends(get_db)):
    """Récupérer l'état de visibilité du système de paiement"""
    try:
        role, username, user_data = require_auth(request, db)
        if not universite_id:
            universite_id = user_data.get('universite_id') if isinstance(user_data, dict) else getattr(user_data, 'universite_id', None)
        if not universite_id:
            return {"masque": False}
        p = _get_or_create_parametre(db, universite_id)
        return {"masque": getattr(p, 'paiement_masque', False)}
    except HTTPException:
        raise
    except Exception:
        return {"masque": False}

@app.post("/api/parametres/paiement/toggle")
async def toggle_paiement(request: Request, db: Session = Depends(get_db)):
    """Basculer la visibilité du système de paiement (mode gratuit)"""
    role, username, user_data = require_auth(request, db)
    if role != "admin":
        raise HTTPException(status_code=403, detail="Accès réservé aux administrateurs")
    universite_id = None
    ...
    if not universite_id:
        raise HTTPException(status_code=400, detail="Université non trouvée")
    p = _get_or_create_parametre(db, universite_id)
    p.paiement_masque = not getattr(p, 'paiement_masque', False)
    db.commit()
    return {
        "success": True,
        "masque": p.paiement_masque,
        "message": f"Paiement {'masqué (accès gratuit)' if p.paiement_masque else 'visible (accès payant)'}"
    }
